...
int64 nChainStartTime = 1509341195;
...
const string strMessageMagic = "ICBCoin Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ICBCoin
    bool fStrictPayToScriptHash = true; // Always active in ICBCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ICBCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "A block chain  vest for ICB, Created in Thailand，For the world 2017-10-30";
        CTransaction txNew;
        txNew.nTime = nChainStartTime;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 5261126560 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        block.nNonce   = !fTestNet ? 252158 : 691799;
        ...
        printf("blockGenesis.nBits = %u\n", block.nBits);
        printf("blockGenesis.nNonce = %u\n", block.nNonce);
		}

        printf("block.CURRENT_VERSION()1 == %lld\n", block.CURRENT_VERSION);
        printf("block.GetHash()2== %lld\n", block.nVersion);
        printf("block.GetHash()3 == %s\n", block.GetHash().ToString().c_str());
        printf("block.hashMerkleRoot == %s\n", block.hashMerkleRoot.ToString().c_str());
        printf("block.nTime = %u \n", block.nTime);
        printf("block.nNonce = %u \n", block.nNonce);
        block.print();
        assert(block.hashMerkleRoot == uint256("0x12c710391889405079bb1cd8d8b395c3fb706acfcd30b44d84d984bec1ec74af"));
...
unsigned char pchMessageStart[4] = { 0xaf, 0xb7, 0xd2, 0x9f };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv);//Fixed initial download blocked every 500 PoS block #110
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                       ...
                       // vInv.push_back(CInv(MSG_BLOCK, hashBestChain));//Fixed initial download blocked every 500 PoS block #110
